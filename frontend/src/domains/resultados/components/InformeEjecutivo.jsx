...
import { obtenerEstadoIA, obtenerInforme, getErrorIA } from '../../../shared/services/analisisService'
...
export default function InformeEjecutivo() {
    ...
    async function generar() {
        ...
        setError(null)
        try {
            setInforme(await obtenerInforme())
        } catch (err) {
        ...
        <section className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="flex items-start justify-between gap-3 flex-wrap p-5 no-print">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        <h2 className="text-base font-semibold text-[#1e3654]">Informe ejecutivo</h2>
                        <p className="text-xs text-gray-500 mt-0.5">
                            Estado institucional completo, listo para presentar a la alta dirección.
                        </p>
                    </div>
                </div>
            ...
            {cargando && !informe && (
                <p className="px-5 pb-5 text-sm text-gray-400 text-center no-print">
                    Consolidando el estado de todos los módulos y redactando el informe…
                </p>
            )}
